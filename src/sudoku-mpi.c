...
int solve_from(int *sudoku, int *cp_sudoku, uint64_t *rows_mask, uint64_t *cols_mask, uint64_t *boxes_mask, List *work,
               int last_pos);
...
int *read_matrix(const char *input_file_path);
...
static const char *const usage[] = {
        "mpiexec -np 2 ./sudoku --result=./solve_this_sudoku.txt",
        NULL
};
...
int main(int argc, const char **argv) {
    ...
    struct argparse_option options[] = {
            OPT_GROUP("Options:"),
            OPT_HELP(),
            OPT_STRING(0, "input", &input_file_path, "Path to sudoku input file", NULL, 0, OPT_NONEG),
            OPT_END(),
    };
    ...
    if (input_file_path == NULL || input_file_path[0] == '\0') {
        fprintf(stderr, "Invalid input file path: --input required\n");
        return EXIT_FAILURE;
    }

    int *sudoku, result, total;
    sudoku = read_matrix(input_file_path);
        ...
        while (work->head != NULL) {
            ...
            while (1) {
                ...
                if (flag && status.MPI_TAG != -1) {
                    ...
                    MPI_Recv(number_buf, number_amount, MPI_INT, status.MPI_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD,
                             &status);
...
void
delete_from(int *sudoku, int *cp_sudoku, uint64_t *rows_mask, uint64_t *cols_mask, uint64_t *boxes_mask, int cell) {
    int i;
...
int exists_in(int index, uint64_t *mask, int num) {
    ...
    if (res !=
        mask[index])           // since the result is different from the initial mask, it is safe to put 3 in this row/col/box
        return 0;
    return 1;
}
...
int is_safe_num(uint64_t *rows_mask, uint64_t *cols_mask, uint64_t *boxes_mask, int row, int col, int num) {
    return !exists_in(row, rows_mask, num) && !exists_in(col, cols_mask, num) &&
           !exists_in(BOX(row, col), boxes_mask, num);
}
